...
int main()
{
    ...
    // Read the file
    cout << "\nPlease enter the name of the file (don't include the .txt): ";
    ...
    // Parse the tree
        while (token_ptr < temp.size())
        {
            Token tk = temp[token_ptr];
            ...
            cout << "Token: " << tk.lexemeName << "\t\t Lexeme: " << tk.lexeme << endl;
            isExpression(tk);
...
void backup()
{
    token_ptr > 0 ? token_ptr -- : token_ptr = token_ptr;
};
