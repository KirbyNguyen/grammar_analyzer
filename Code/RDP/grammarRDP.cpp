...
int main()
{
    ...
    while (token_ptr < tokens.size())
    {
        ...
        // If it is duing a <STATEMENT>
        if (duringStatement)
        {
            // Ends the <STATEMENT> if it ends with ";"
            if (tokens[token_ptr].lexeme == ";")
            {
                duringStatement = false;
                duringExpression = false;
            }
            // Check if it is during an <EXPRESSION>
            else if (duringExpression)
            {
                // Check for <TERM>
                if (checkTerm()) {
                    
                // If it is not a <TERM>, it should be a <TERMPRIME> or <EXPRESSIONPRIME>
